...
def login():
    ...
    if user_id:
        ...
        open_manager(user_id)  # Pass the user ID to open_manager
        root.destroy()
    else:
        status_label.config(text="Invalid username or password", foreground="red")
def open_manager(user_id):
    file_path = 'manage.py'
    try:
        subprocess.Popen(['python', file_path, str(user_id)])  # Pass user_id as an argument
